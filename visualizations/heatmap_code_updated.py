...
data = {
    'Country': [
        ...
        'Iran', 'Egypt', 'Saudi Arabia', 'United Arab Emirates', 'Libya', 'Syria', 'Tunisia', 'Algeria', 'Morocco', 'Iraq',
        # Sub-Saharan Africa
...
# Load world map data from the downloaded GeoJSON file
world = gpd.read_file('/home/ubuntu/internet_censorship_report/visualizations/ne_110m_admin_0_countries.geojson')

# Rename the country name column to match our data
world = world.rename(columns={'ADMIN': 'Country'})

# Merge the data with the world map
world = world.merge(df, how='left', left_on='Country', right_on='Country')
...
# Define region bounding boxes (approximate lat/lon bounds)
region_bounds = {
    'Asia Pacific': {'minx': 60, 'miny': -10, 'maxx': 180, 'maxy': 55},
    'Middle East & North Africa': {'minx': -10, 'miny': 10, 'maxx': 60, 'maxy': 45},
    'Sub-Saharan Africa': {'minx': -20, 'miny': -35, 'maxx': 55, 'maxy': 15},
    'Eurasia': {'minx': 20, 'miny': 35, 'maxx': 90, 'maxy': 75},
    'Americas': {'minx': -170, 'miny': -60, 'maxx': -30, 'maxy': 80}
}
...
for region in regions:
    ...
    # Plot all countries for context but with focus on the region
    world.plot(color='lightgray', ax=ax)
    
    # Filter world data for this region
    region_countries = region_df['Country'].tolist()
    region_map = world[world['Country'].isin(region_countries)]
    
    # Plot the region countries with color based on censorship score
    ...
    # Set the map bounds to focus on the region
